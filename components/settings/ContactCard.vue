  ...
  <UiCard v-for="card in props.cards" :key="card.id" class="w-full dark:bg-[#202020]">
    <UiCardHeader class="flex flex-row items-start gap-2 space-y-0 p-4">
      ...
      <div class="space-y-px">
        <UiCardTitle class="text-base">{{ $t(`${card.title}`) }}</UiCardTitle>
        <UiCardDescription class="text-sm text-zinc-400">
          <UiBadge variant="secondary">
            <span class="text-xs text-zinc-400 dark:text-zinc-200">{{ card.who }}</span>
          </UiBadge>
        </UiCardDescription>
      </div>
    </UiCardHeader>
    <UiCardContent class="pb-1 pl-4 pr-4">
      <div class="flex items-center space-x-4 text-xs text-zinc-500">
        {{ locale == 'en-US' || locale == 'hi-IN' ? card.description : card.description }}
      </div>
    </UiCardContent>
